...
class TestAcsTweak(BaseACS):
    ...
    def test_tweak(self):
        ...
        # Prepare input files.
        input_file1 = self.get_data('input', input_file1)
        input_file2 = self.get_data('input', input_file2)
    ...
    def test_pixsky1(self):
        ...
        input_file = self.get_data('input', 'j94f05bgq_flt.fits')
        ...
        for i in [1,2]:
            ...
            pixtopix.tran(froot+'[sci,%d]'%i,'{}[sci,1]'.format(refwcs_file),
                        direction='forward',
                        x=None,y=None,coordfile=incat,colnames='',separator=None,
                        precision=6,output=outcat,verbose=False)
            ...
            pixtosky.xy2rd(froot+'[sci,%d]'%i, coordfile=incat,
                        ...
                        hms=True, precision=6, output=outskycat,verbose=False)
            shutil.copyfileobj(open(outskycat,'r'),outsky)

            outpixcat = froot.replace('_flt.fits','_flt_sci%d_skyxy.coo'%i)
            skytopix.rd2xy(froot+'[sci,%d]'%i,ra=None,dec=None,coordfile=inskycat,
                            colnames=None,precision=6,output=outpixcat,verbose=False)
