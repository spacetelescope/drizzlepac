...
def run(configObj, wcsmap=None):
    ...
    if scale_pars["in_units"] == "counts":
        if scale_pars["expkey"] in _inimg["PRIMARY"].header:
            ...
            raise ValueError(
                "No valid exposure time keyword could be found "
    ...
    if os.path.exists(_refname):
        ...
        if blot_wcs.wcs.is_unity():
            print(
                "WARNING: No valid WCS found for output image: {} !".format(
                    configObj["reference"]
                )
            )
...
def buildBlotParamDict(configObj):
    ...
    paramDict = {
        "blot_interp": configObj[blot_name]["blot_interp"],
    ...
    for img in imageObjectList:
        ...
        for chip in img.returnAllChips(extname=img.scienceExt):
            ...
            if skyval is not None:
                _outsci += skyval
                log.info(
                    "Applying sky value of %0.6f to blotted image %s"
