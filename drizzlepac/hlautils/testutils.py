...
def compare_wcs_alignment(dataset, force=False):
    """ Return results from aligning dataset using all available WCS solutions.
        ...
        This code will ALWAYS make sure the ``ASTROMETRY_STEP_CONTROL``
        variable is set to "ON" when running and will reset to the original
        state when completed. This insures that the code ALWAYS queries the
        astrometry database to apply all avaialable a priori WCS solutions.
    ...
    try:
        # Step 1: Determine alignment for pipeline-defined WCS
        results = alignimages.perform_align([dataset],
                                            clobber=force, debug=True)
        if not results:
            msg = """
            No valid exposures found for {}
        ...
        # Step 2: Create results output organized by WCSNAME
        ...
        # Step 3: Update inputs with latest distortion model and pull in
        #         solutions from dB
        imglist = updatewcs.updatewcs(imglist)
        img0 = imglist[0]

        # Step 4: Loop over each WCS solution and perform alignment to GAIA
        ...
        for wcs in wcsnames:
            ...
            if 'OPUS' in wcs or wcs == default_wcsname:
                # skip default pipeline solutions, since we have already
                # aligned it
                continue
            ...
            for img in imglist:
                ...
                if wcs in wnames:
                    ...
                    headerlet.restore_from_headerlet(
                        img, hdrname=hdrname, archive=False, force=True
                    )

            results = alignimages.perform_align(
                [dataset], clobber=False, debug=True
            )
        ...
        if control is None:  # Need to be explicit here since T/F are valid
...
def extract_results(results):
    ...
    results_dict = {
        'images': results['imageName'].astype(str).tolist(),
        'offset_x': results['offset_x'],
        'offset_y': results['offset_y'],
        'rotation': results['rotation'],
        'scale': results['scale'],
        'rms_x': results['rms_x'],  # in pixels
        'rms_y': results['rms_y'],
        'fit_rms': results['fit_rms'],  # in arcsec
