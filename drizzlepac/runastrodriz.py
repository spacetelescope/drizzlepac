...
focus_pars = {
    "WFC3/IR": {"sigma": 2.0, "good_bits": 512},
...
def process(
    ...
    if not wfpc2_input:
        ...
        if _analyze_exposure(inFilename):
            ...
            print(
                "ERROR: Inappropriate input file.  Deleting converted WFPC2 FLT file."
            )
            return
    ...
    if force:
        ...
        if _mname:
            _fimg = fits.open(
                fileutil.buildRootname(_mname, ext=[raw_suffix]), memmap=False
            )
    ...
    if dcorr == "PERFORM":
        ...
        _good_images = [f for f in _calfiles if fits.getval(f, "exptime") > 0.0]
        _good_images = [
            f
            for f in _good_images
            if fits.getval(f, goodpix_name, ext=("SCI", 1)) > 0.0
        ]
        ...
        if align_with_apriori or force_alignment or align_to_gaia:
            ...
            align_dicts, align_table = verify_alignment(
                _inlist,
                _calfiles,
                _calfiles_flc,
                _trlfile,
        ...
        if align_with_apriori:
            ...
            _wnames_calfiles = [
                (c, fits.getval(c, "wcsname", ext=1)) for c in _calfiles
            ]
            ...
            try:
                ...
                # Generate initial default products and perform verification
                align_apriori, apriori_table = verify_alignment(
                    _inlist,
                    _calfiles,
                    _calfiles_flc,
                    _trlfile,
        ...
        if align_to_gaia:
            ...
            sub_dirs.append(tmpname)
            align_aposteriori, aposteriori_table = verify_alignment(
                _inlist,
                _calfiles,
                _calfiles_flc,
                _trlfile,
        ...
        drz_products, asn_dicts, diff_dicts = run_driz(
            _inlist,
            _trlfile,
            _calfiles,
    ...
    for fname in _calfiles:
        ...
        updatewcs_hdrlets = sorted(
            glob.glob(f'{fname.replace(".fits", "")}_??????_hlet.fits')
        )
    ...
    if not debug:
        ...
        try:
            ...
            print(
                "WARNING: Unable to remove any or all of these sub-directories: \n{}\n".format(
                    sub_dirs
                )
            )
...
def run_driz(
    ...
    **pipeline_pars,
    ...
    drizlog = (
        pipeline_pars["runfile"] + ".log"
    )  # the '.log' gets added automatically by astrodrizzle
    for infile in inlist:  # Run astrodrizzle for all inputs
        asndict, ivmlist, drz_product = processInput.process_input(
            infile, updatewcs=False, preserve=False, overwrite=False
        )
        del ivmlist
        ...
        try:
            drizzlepac.astrodrizzle.AstroDrizzle(
                input=infile, configobj=None, **pipeline_pars
            )
        ...
        if verify_alignment and asndict is not None:
            ...
            single_files = [
                calfile.replace(cal_suffix, "_single_sci") for calfile in calfiles
            ]
            ...
            instr_det = "{}/{}".format(
                fits.getval(sfile, "instrume"), fits.getval(sfile, "detector")
            )
            focus_sigma = focus_pars[instr_det]["sigma"]
            print(
                "Measuring similarity and focus for: \n{} \n    {}".format(
                    single_files, drz_product
                )
            )
            focus_dicts.append(
                amutils.build_focus_dict(single_files, drz_product, sigma=focus_sigma)
            )
            if debug:
            ...
            diff_dicts[drz_product] = amutils.max_overlap_diff(
                drz_footprint.total_mask, single_files, drz_product, scale=1
            )

        else:
            focus_dicts = None

    # # recompute s_region for drizzled products
    # import ipdb
    ...
    **pipeline_pars,
    ...
    try:
        ...
        if alignment_mode == "aposteriori":
            ...
            try:
                full_table = align.perform_align(
                    alignfiles,
            ...
            if align_update_files and align_table:
                ...
                for fltfile, flcfile in zip(align_update_files, alignfiles):
                    ...
                    if headerlet_file not in ["None", ""]:
                        ...
                        _trlmsg += (
                            "No absolute astrometric headerlet applied to {}\n".format(
                                fltfile
                            )
                        )
        ...
        drz_products, focus_dicts, diff_dicts = run_driz(
            inlist,
            trlfile,
            calfiles,
            mode=tmpmode,
        ...
        if focus_dicts is not None:
            ...
            align_focus = (
                focus_dicts[-1]
                if "drc" in focus_dicts[-1]["prodname"]
                else focus_dicts[0]
            )
            ...
            diff_verification, max_diff = amutils.evaluate_overlap_diffs(
                diff_dicts[dkeys[-1]]
            )
            _trlmsg += "Fraction of sources matched: {}  out of {} sources\n".format(
                fraction_matched, num_sources
            )
            ...
            if fraction_matched < 0.1 or -1 < num_sources < 10:
                ...
                alignment_verified = (
                    True if (diff_verification and focus_verification) else False
                )
            ...
            if alignment_verified:
                ...
                _trlmsg += (
                    "Focus verification indicated that {} alignment FAILED.\n".format(
                        tmpmode
                    )
                )
...
def apply_headerlet(filename, headerlet_file, flcfile=None):
    ...
    headerlet.apply_headerlet_as_primary(
        filename, headerlet_file, attach=True, archive=True
    )
        ...
        with fits.open(f, mode="update") as fhdu:
            ...
            for sciext in range(num_sci):
                ...
                if "-FIT" in wcsname:
                    ...
                    wcsdates = [
                        fits.getval(fileutil.osfn(idc), "date") for idc in idctabs
                    ]
                    ...
                    if most_recent_wcs:
                        ...
                        headerlet.restore_from_headerlet(
                            fhdu, force=True, hdrname=most_recent_wcs[1], archive=False
                        )
                        # insure IDCSCALE is still present
                        if "idcscale" not in fhdu[("sci", 1)].header:
                            msg += "Headerlet {} was missing IDCSCALE keyword".format(
                                most_recent_wcs[1]
                            )
                            # get IDCTAB name
                            itabroot = (
                                fhdu[0].header["idctab"].split("$")[1].split("_")[0]
                            )
                            fhdu_idscale = None
                            ...
                            for extn in extvers:
                                if itabroot in fhdu[("HDRLET", extn)].header["wcsname"]:
                                    ...
                                    if "idcscale" in _hdrlet[("SIPWCS", 1)].header:
                                        fhdu_idscale = _hdrlet[("SIPWCS", 1)].header[
                                            "idcscale"
                                        ]
                                        break
                            if fhdu_idscale is None:
                                ...
                                fhdu_idscale = round(
                                    np.sqrt(np.power(cd11, 2) + np.power(cd21, 2))
                                    * 3600.0,
                                    3,
                                )
                            # Set the value of the IDCSCALE keyword
                            for extn in range(num_sci):
                                msg += "Adding IDCSCALE {} to {}[sci,{}]".format(
                                    fhdu_idscale, fhdu.filename(), extn + 1
                                )
                                fhdu[("sci", extn + 1)].header[
                                    "idcscale"
                                ] = fhdu_idscale
    return msg
...
def update_wcs_in_list(exp_list, logfile=None):
    ...
    print(msg)
    update_msg = msg
    primary_wcsnames = set(
        [fits.getval(fname, "wcsname", ext=("SCI", 1)) for fname in exp_list]
    )

    if len(primary_wcsnames) == 1:
        msg = "\nAll Primary WCS's confirmed as consistent as {}.".format(
            primary_wcsnames
        )
        print(msg)
        update_msg += msg
    ...
    for filename in exp_list:
        ...
        primary_idctabs.append(
            "IDC_{}*".format(idcroot)
        )  # Add all WCS that start with IDC_<idctab> as desired WCSs
    ...
    (
        final_wcs_set,
        skip_direct_list,
        d_keyword_wcs_names_dict,
        direct_dict,
    ) = collect_wcs_names(exp_list, "DIRECT", logfile=logfile)
    ...
    if final_wcsname:
        ...
        for filename in exp_list:
            if filename not in skip_direct_list:
                msg = "\nSetting the primary WCS for direct image {} to {}.".format(
                    filename, final_wcsname
                )
                print(msg)
                update_msg += msg
...
def collect_wcs_names(exp_list, image_type, logfile=None):
    ...
    for filename in exp_list:
        ...
        headerlet_wcs_names = wcsutil.headerlet.get_headerlet_kw_names(
            filename, kw="WCSNAME"
        )
        ...
        # If there are no WCS solutions, the image could be bad (e.g., EXPTIME=0 or EXPFLAG="TDF-DOWN...")
            ...
            msg = "WARNING: There are no WCS solutions in the image {} in this visit.".format(
                filename
            )
            print(msg)
            update_msg += msg
...
def update_active_wcs(filename, wcsname, logfile=None):
    ...
    if key != " ":
        ...
        headerlet_hdrnames = wcsutil.headerlet.get_headerlet_kw_names(
            filename, kw="HDRNAME"
        )
        ...
        if headerlet_hdrnames:
            ...
            for wkey, wname in wcs_key_dict.items():
                ...
                if index == -1 and wkey.upper() != "O":
                    ...
                    hdr_keyword = fits.getval(
                        filename, "HDRNAME{}".format(wkey.upper()), ext=1
                    )
            ...
            headerlet_wcsnames = wcsutil.headerlet.get_headerlet_kw_names(
                filename, kw="WCSNAME"
            )
            ...
            extensions = []
            extensions = wcsutil.headerlet.find_headerlet_HDUs(
                filename, hdrname=hdrname
            )
            ...
            for ext in reversed(extensions[1:]):
                ...
                msg += (
                    "Delete duplicate headerlet extension {} in filename {}.\n".format(
                        ext, filename
                    )
                )
            ...
            try:
                wcsutil.headerlet.restore_from_headerlet(
                    filename, hdrname=hdrname, force=True
                )
                # Update value of nmatches based on headerlet
                fhdu = fits.open(filename, mode="update")
                for sciext in range(1, num_sci_ext + 1):
                    nm = (
                        fhdu[extensions[0]].header["nmatch"]
                        if "nmatch" in fhdu[extensions[0]].header
                        else 0
                    )
                ...
                if found_string:
                    wcsutil.altwcs.restoreWCS(
                        filename, ext=extname_list, wcsname=found_string[0]
                    )
                else:
    ...
    for wname, extname in zip(hdrlet_wcsnames, hdrlet_extensions):
        ...
        if "-FIT" in wname:
            ...
            if not valid_ctype or not valid_dist_kws:
                ...
                invalid_extns.append(
                    {"extname": extname, "wcsname": wname, "key": keyname}
                )
...
def _update_wcs_fit_keywords(fltfile, flcfile):
    ...
    hdulist[0].header.set(
        "UPWCSVER",
        value=hdulist_flc[0].header["UPWCSVER"],
    ...
    hdulist[0].header.set(
        "PYWCSVER",
        value=hdulist_flc[0].header["PYWCSVER"],
...
def rmtree2(path, n=3):
    ...
    if not ok:
        print(
            "Failed to remove path %s with shutil.rmtree, even after %d attempts.".format(
                path, n
            )
        )
        raise OSError
    ...
    if help:
        ...
        try:
            process(
                ...
                num_cores=num_cores,
                inmemory=inmemory,
                headerlets=headerlets,
                align_to_gaia=align_to_gaia,
                force_alignment=force_alignment,
                do_verify_guiding=do_verify_guiding,
                debug=debug,
