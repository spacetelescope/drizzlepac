...
def comparesourcelists(slNames=None, imgNames=None, good_flag_sum = 255, plotGen=None, plotfile_prefix=None,
    ...
    if input_json_filename:
        ...
        if output_json_filename:
            ...
            diag_obj.add_update_info_item("reference catalog filename", slNames[0])
            diag_obj.add_update_info_item("comparison catalog filename", slNames[1])
    ...
    if input_json_filename:
        ...
        if output_json_filename:  # Add matched values to diag_obj
            ...
            diag_obj.add_update_info_item("plate_scale", plate_scale)
    ...
    if matched_values_ra.shape[1] > 0 and matched_values_ra.shape[1] == matched_values_dec.shape[1]:
        ...
        if input_json_filename:
            ...
            if output_json_filename:  # Add 'ref_frame' and 'comp_frame" values to header so that will SkyCoord() execute OK
                diag_obj.add_update_info_item("ref_frame", ref_frame)
                diag_obj.add_update_info_item("comp_frame", comp_frame)
