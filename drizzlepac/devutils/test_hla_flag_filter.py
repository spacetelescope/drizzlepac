...
def preserve_orig_files(hff_inputs,source_path,dest_path):
...
def run_stuff(input_pickle_filename, qc_json_filename):
    ...
    preserve_orig_files(hff_inputs,os.getcwd(), temp_dirname)


    run_hla_flag_filter(hff_inputs)

    run_compare_sourcelists(hff_inputs, log_level)
    log.info("Moving new files generated by hla_flag_filter.py to final path " + temp_dirname)
    preserve_orig_files(hff_inputs,os.getcwd(),final_dir)
    if qc_json_filename:
    ...
    preserve_orig_files(hff_inputs,temp_dirname,os.getcwd())
