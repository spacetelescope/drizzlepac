...
def build_svm_plots(data_source):
    ...
    data_cols = HOVER_COLUMNS + ['distribution_characterization_statistics.Number_of_GAIA_sources',
    ...
    gaia_cols = get_pandas_data(data_source, data_cols)

    gaia_plots = build_gaia_plots(gaia_cols, data_cols)
    

# -----------------------------------------------------------------------------
# Functions for generating each data plot
#    
def build_gaia_plots(gaia_data, data_cols, output_basename='gaia_comparison_svm_qa'):
...
def get_pandas_data(data_source, data_columns):
    ...
    if data_source.endswith(".h5"):
        fit_data = df_handle.get_columns_HDF5(data_columns)
    else:
        fit_data = df_handle.get_columns_CSV(data_columns)
...
def plot_histogram(title, hist, edges, y_start=0, 
                    fill_color='navy', background_fill_color='#fafafa', 
                    xlabel='', ylabel=''):
    p = figure(title=title, tools='', background_fill_color=background_fill_color)
