...
def mvm_combine(cutout_files, img_combiner=None, output_dir=".", log_level=logutil.logging.INFO):
    ...
    # Combined files will be generated for each detector/filter combination
    # Walk the sorted input list and create a group list for each new detector/filter
    temp_list = []
    detector_filter_list = []
    prev_detector = ""
    prev_filter = ""
    for cfile in cutout_files:
        cf = cfile.replace(".fits", "")
        detector = cf.split("_")[3]
        filter = cf.split("_")[4].split("-")[0]
        if detector != prev_detector:
            prev_detector = detector
            prev_filter = filter
            if temp_list and len(temp_list) > 1:
                detector_filter_list.append(temp_list.copy())
            elif temp_list and len(temp_list) <= 1:
                log.warning("There is only one file with this detector/filter combination, so there" \
                            " is nothing to combine.")
                log.warning("File {} will be ignored for combination purposes.".format(temp_list))
            temp_list.clear()
            temp_list.append(cfile)
        elif detector == prev_detector:
            if filter != prev_filter:
                ...
                temp_list.clear()
                temp_list.append(cfile)
            elif filter == prev_filter:
                temp_list.append(cfile)

    # If appropriate, append the last temp_list
    if temp_list and len(temp_list) > 1:
        detector_filter_list.append(temp_list.copy())
    ...
    # Check the detector/filter list has entries - it should be a list of lists
    if detector_filter_list:

        # Process each group of detector/filter files
        for df_group in detector_filter_list:
            ...
            filename = fits.getheader(df_group[0], ext=0)['FILENAME']
            f = cfile.replace(".fits", "")
            tokens = filename.split("_")
            detector =tokens[3]
            filter = tokens[4].split("-")[0]
            sky_tokens = tokens[2].split("-")
            skycell = sky_tokens[1][1:5]
            ra = sky_tokens[2]
            dec = sky_tokens[3]
            output_filename = os.path.join(output_dir, OUTPUT_PREFIX + ra + "-" + dec + "_" + detector + "_" + filter + ".fits")
