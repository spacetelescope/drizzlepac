...
def build_interfilter_crossmatch_plots(xm_df, display_plot, output_basename='svm_qa', log_level=logging.INFO):
    ...
    for colname in columns2convert:
        xm_df[colname] *= xm_df[
            'ref_image_platescale']  # Multiply values in pixels by platescale (arcsec/pixel) to convert to values to arcseconds
    ...
    for line_index in xm_df.index:
        ...
        resid_output = output.replace(".html", "_residuals_{}_{}_{}_{}.html".format(
...
def generate_photometry_graphic(phot_dataDF, output_base_filename='', display_plot=False,
    ...
    for line_index in subphot_dataDF.index:
        ...
        mag_outfile = output_base_filename + '_' + row_DF['Dataset'][0] + '_' + row_DF['gen_info.filter'][
            0] + '.html'
        ...
        noexpand_columns = ['gen_info.imgname', 'gen_info.proposal_id', 'gen_info.filter', 'colormap',
                            'inst_det']
        ...
        expanded_mag_DF['Delta_MagAp1'] = (
                    expanded_mag_DF['Point_MagAp1'] - expanded_mag_DF['Segment_MagAp1']).abs()
        expanded_mag_DF['Delta_MagAp2'] = (
                    expanded_mag_DF['Point_MagAp2'] - expanded_mag_DF['Segment_MagAp2']).abs()
        ...
        p4 = HAPFigure(title='On-sky Separation vs Magnitude (Aperture 1)   Filter: ' +
                             expanded_mag_DF['gen_info.filter'][0],
        ...
        p5 = HAPFigure(title='On-sky Separation vs Magnitude (Aperture 2)   Filter: ' +
                             expanded_mag_DF['gen_info.filter'][0],
        ...
        p6 = HAPFigure(
            title='Delta Magnitude vs Magnitude (Aperture 1)   Filter: ' + expanded_mag_DF['gen_info.filter'][
                0],
            x_label='Point Magnitude Aperture 1 (ABMag)',
        ...
        p7 = HAPFigure(
            title='Delta Magnitude vs Magnitude (Aperture 2)   Filter: ' + expanded_mag_DF['gen_info.filter'][
                0],
            x_label='Point Magnitude Aperture 2 (ABMag)',
            y_label='Delta Magnitude [Point - Segment] (ABMag)',
            hover_tips=mag2_tooltips)
