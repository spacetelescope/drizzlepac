...
def run_hap_processing(input_filename, diagnostic_mode=False, input_custom_pars_file=None,
    ...
    # The mfile_list is the list of manifest files generated by request during runastrodriz.
    #product_list = []
    #mfile_list = []
    ...
    try:
        ...
        #MDD
        log.info("In Exception.  Manifest_name: {} len(mfile_list): {}".format(manifest_name, len(mfile_list)))
        return_value = 1
        print("\a\a\a")
        exc_type, exc_value, exc_tb = sys.exc_info()
        traceback.print_exception(exc_type, exc_value, exc_tb, file=sys.stdout)
    ...
    # This except handles sys.exit() which raises the SystemExit exception which inherits from BaseException.
        ...
        #MDD
        log.info("In BaseException.  len(mfile_list): {}".format(len(mfile_list)))
        exc_type, exc_value, exc_tb = sys.exc_info()
