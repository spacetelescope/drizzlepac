...
log = logutil.create_logger(
    "test_alignpipe_randomlist", level=logutil.logging.INFO, stream=sys.stdout
)
...
def pytest_generate_tests(metafunc):
    ...
    print(
        "\nTEST_RANDOM. Start row: {}   Number of rows to process: {}.".format(
            start_row, num_rows
        )
    )
...
def test_alignpipe_randomlist(tmpdir, dataset):
    ...
    try:
        ...
        files_on_disk = check_disk_get_data(
            [dataset], suffix=input_parameter, archive=False, clobber=False
        )
        ...
        jref_dir = (
            "jref.old/"
            if "16r12191j_mdz" in fits.getval(flts[0], "mdriztab")
            else "jref/"
        )
