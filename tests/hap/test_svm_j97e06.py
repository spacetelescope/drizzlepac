...
def test_svm_point_cat_numsources(gather_output_data):
    ...
    cat_files = [
        files
        for files in gather_output_data
        if files.lower().endswith("point-cat.ecsv")
    ]
    ...
    for cat in EXPECTED_POINT_SOURCES.keys():
        for file in cat_files:
            if cat in file and "total" in file:
                valid_cats[cat] = (
                    np.isclose(num_sources[file], EXPECTED_POINT_SOURCES[cat], atol=1),
                    num_sources[file],
                )
                break
    ...
    assert (
        len(bad_cats) == 0
    ), f"Point Catalog(s) {bad_cats} had {valid_cats} sources, expected {EXPECTED_POINT_SOURCES}"
...
def test_svm_segment_cat_numsources(gather_output_data):
    ...
    for cat in EXPECTED_SEG_SOURCES.keys():
        for file in cat_files:
            if cat in file and "total" in file:
                valid_cats[cat] = (
                    np.isclose(num_sources[file], EXPECTED_SEG_SOURCES[cat], atol=1),
                    num_sources[file],
                )
                break
    bad_cats = [cat for cat in valid_cats if not valid_cats[cat][0]]
    assert (
        len(bad_cats) == 0
    ), f"Segment Catalog(s) {bad_cats} had {valid_cats} sources, expected {EXPECTED_SEG_SOURCES}"
...
def test_svm_point_cat_meanmag(gather_output_data):
    ...
    for cat in MEAN_CAT_MAGAP1_POINT.keys():
        for file in cat_files:
            if cat == file:
                good_cats[cat] = (
                    np.isclose(
                        MEAN_CAT_MAGAP1_POINT[cat], Mag1_mean[cat], rtol=POINT_DIFF
                    ),
                    Mag1_mean[cat],
                )
                break
    ...
    assert (
        len(bad_cats) == 0
    ), f"Point Catalog(s) {bad_cats} had {good_cats} sources, expected {MEAN_CAT_MAGAP1_POINT}"
...
def test_svm_segment_cat_meanmag(gather_output_data):
    ...
    for cat in MEAN_CAT_MAGAP1_SEGMENT.keys():
        for file in cat_files:
            if cat == file:
                good_cats[cat] = (
                    np.isclose(
                        MEAN_CAT_MAGAP1_SEGMENT[cat], Mag1_mean[cat], rtol=SEGMENT_DIFF
                    ),
                    Mag1_mean[cat],
                )
                break
    ...
    assert (
        len(bad_cats) == 0
    ), f"Segment Catalog(s) {bad_cats} had {good_cats} sources, expected {MEAN_CAT_MAGAP1_SEGMENT}"
