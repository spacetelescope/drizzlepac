...
log = logutil.create_logger(
    "test_randomlist", level=logutil.logging.INFO, stream=sys.stdout
)
...
def test_randomlist(tmpdir, dataset, request):
    ...
    try:
        align_table = alignimages.perform_align(
            [dataset],
        ...
        if align_table is None:
            mark = pytest.mark.xfail(
                reason="TEST_RANDOM. There were not enough sources for processing for dataset."
            )
            ...
            # Datasets to process
                ...
                if fit_qual > 4:
                    mark = pytest.mark.xfail(
                        reason="TEST_RANDOM. Unsuccessful Dataset (fit_qual = 5)."
                    )
                    request.node.add_marker(mark)
                    assert 0
                else:
