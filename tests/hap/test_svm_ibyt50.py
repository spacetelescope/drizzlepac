...
def test_svm_point_total_cat(gather_output_data):
    ...
    assert (
        len(bad_cats) == 0
    ), f"Total Point Catalog(s) {bad_cats} had {valid_cats} sources, expected {expected_point_sources}"
...
def test_svm_segment_total_cat(gather_output_data):
    ...
    assert (
        len(bad_cats) == 0
    ), f"Total Segment Catalog(s) {bad_cats} had {valid_cats} sources, expected {expected_segment_sources}"
