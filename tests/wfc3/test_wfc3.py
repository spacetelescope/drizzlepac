...
class TestWFC3(BaseWFC3):
    ...
    def test_uvis_single(self):
        ...
        astrodrizzle.AstroDrizzle(
            driz_input, mdriztab=False, configobj=parObj, **input_pars
        )
    ...
    def test_uvis_asn(self):
        ...
        parObj = teal.load("astrodrizzle", defaults=True)  # get all default values
        astrodrizzle.AstroDrizzle(
            asn_file, mdriztab=False, configobj=parObj, **input_pars
        )
    ...
    def test_wfc3_ir_saturated(self):
        # Prepare input files.
        raw_inputs = ["ib5w02n0q_flt.fits", "ib5w02naq_flt.fits", "ib5w02njq_flt.fits"]
        inputs = [os.path.basename(self.get_input_file("input", i)) for i in raw_inputs]
        ...
        parObj = teal.load("astrodrizzle", defaults=True)  # get all default values
        astrodrizzle.AstroDrizzle(
            inputs, mdriztab=False, configobj=parObj, **input_pars
        )
