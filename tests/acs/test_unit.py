...
class TestDriz(BaseUnit):
    def test_square_with_point(self):
        ...
        (min_diff, med_diff, max_diff) = self.centroid_statistics(
            "square with point", output_difference, outsci, template_data, 20.0, 8
        )
    ...
    def test_square_with_grid(self):
        ...
        output_template = os.path.basename(
            self.get_data("truth", "reference_square_grid.fits")
        )
        ...
        (min_diff, med_diff, max_diff) = self.centroid_statistics(
            "square with grid", output_difference, outsci, template_data, 20.0, 8
        )
    ...
    def test_turbo_with_grid(self):
        ...
        output_difference = os.path.basename(
            self.get_data("truth", "difference_turbo_grid.txt")
        )
        output_template = os.path.basename(
            self.get_data("truth", "reference_turbo_grid.fits")
        )
        ...
        (min_diff, med_diff, max_diff) = self.centroid_statistics(
            "turbo with grid", output_difference, outsci, template_data, 20.0, 8
        )
    ...
    def test_gaussian_with_grid(self):
        ...
        output_difference = os.path.basename(
            self.get_data("truth", "difference_gaussian_grid.txt")
        )
        output_template = os.path.basename(
            self.get_data("truth", "reference_gaussian_grid.fits")
        )
        ...
        (min_diff, med_diff, max_diff) = self.centroid_statistics(
            "gaussian with grid", output_difference, outsci, template_data, 20.0, 8
        )
    ...
    def test_lanczos_with_grid(self):
        ...
        output_difference = os.path.basename(
            self.get_data("truth", "difference_lanczos_grid.txt")
        )
        output_template = os.path.basename(
            self.get_data("truth", "reference_lanczos_grid.fits")
        )
        ...
        (min_diff, med_diff, max_diff) = self.centroid_statistics(
            "lanczos with grid", output_difference, outsci, template_data, 20.0, 8
        )
    ...
    def test_tophat_with_grid(self):
        ...
        (min_diff, med_diff, max_diff) = self.centroid_statistics(
            "tophat with grid", output_difference, outsci, template_data, 20.0, 8
        )
    ...
    def test_point_with_grid(self):
        ...
        output_difference = os.path.basename(
            self.get_data("truth", "difference_point_grid.txt")
        )
        output_template = os.path.basename(
            self.get_data("truth", "reference_point_grid.fits")
        )
        ...
        (min_diff, med_diff, max_diff) = self.centroid_statistics(
            "point with grid", output_difference, outsci, template_data, 20.0, 8
        )
...
class TestBlot(BaseUnit):
    ...
    def test_blot_with_point(self):
        ...
        output_difference = os.path.basename(
            self.get_data("truth", "difference_blot_point.txt")
        )
        output_template = os.path.basename(
            self.get_data("truth", "reference_blot_point.fits")
        )
        ...
        (min_diff, med_diff, max_diff) = self.centroid_statistics(
            "blot with point", output_difference, outsci, template_data, 20.0, 16
        )
    ...
    def test_blot_with_default(self):
        ...
        output_difference = os.path.basename(
            self.get_data("truth", "difference_blot_default.txt")
        )
        output_template = os.path.basename(
            self.get_data("truth", "reference_blot_default.fits")
        )
        ...
        (min_diff, med_diff, max_diff) = self.centroid_statistics(
            "blot with defaults", output_difference, outsci, template_data, 20.0, 16
        )
    ...
    def test_blot_with_lan3(self):
        ...
        output_difference = os.path.basename(
            self.get_data("truth", "difference_blot_lan3.txt")
        )
        output_template = os.path.basename(
            self.get_data("truth", "reference_blot_lan3.fits")
        )
        ...
        expin = 1.0
        outsci = ablot.do_blot(
            insci, input_wcs, output_wcs, expin, interp="lan3", coeffs=False
        )
        ...
        (min_diff, med_diff, max_diff) = self.centroid_statistics(
            "blot with lan3", output_difference, outsci, template_data, 20.0, 16
        )
    ...
    def test_blot_with_lan5(self):
        ...
        output_difference = os.path.basename(
            self.get_data("truth", "difference_blot_lan5.txt")
        )
        output_template = os.path.basename(
            self.get_data("truth", "reference_blot_lan5.fits")
        )
        ...
        expin = 1.0
        outsci = ablot.do_blot(
            insci, input_wcs, output_wcs, expin, interp="lan5", coeffs=False
        )
        ...
        (min_diff, med_diff, max_diff) = self.centroid_statistics(
            "blot with lan5", output_difference, outsci, template_data, 20.0, 16
        )
    ...
    def test_blot_with_image(self):
        ...
        output_template = os.path.basename(
            self.get_data("truth", "reference_blot_image.fits")
        )
