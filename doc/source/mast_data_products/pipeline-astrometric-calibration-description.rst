...
considered compromised. These checks include:
  ...
  * **large_rms_check**: Ensures that both the fit_rms and the max_rms are less 
    than the max_fit_limit (default is 150). The fit_rms is RMS between the 
    astrometric catalog and the image alignment sources for an individual 
    exposure. The max_rms is the maximum of the fit_rms values for all the 
  ...
  * **nmatches_check**: Ensures that the number of matches between the image 
    ...
    (e.g. acs_hrc_alignment_all.json). The nmatch requirement values are those 
    next to each geometry in the mosaic_fitgeom_list and mosaic_relgeom_list 
    values entries. Alternatively, the check passes so long as the number of 
    cross matches is at least 2, and the fit_rms is less than 0.5. 
